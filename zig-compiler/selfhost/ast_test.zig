...
// zbr:selfhost/ast_test.zbr:9
        self.testSpan();
// zbr:selfhost/ast_test.zbr:10
        self.testNamedTypeRef();
// zbr:selfhost/ast_test.zbr:11
        self.testExprIntLit();
// zbr:selfhost/ast_test.zbr:12
        self.testBinaryOp();
...
// zbr:selfhost/ast_test.zbr:28
    ...
    pub fn testExprIntLit(self: *AstTest) void {
        _ = self;
// zbr:selfhost/ast_test.zbr:31
        const s = ast.Span.init(3, 1, 3, 2);
// zbr:selfhost/ast_test.zbr:32
...
// zbr:selfhost/ast_test.zbr:33
    ...
    pub fn testBinaryOp(self: *AstTest) void {
        _ = self;
// zbr:selfhost/ast_test.zbr:37
        const op = ast.BinaryOp.add;
// zbr:selfhost/ast_test.zbr:38
        switch (op) {
            .add => {
                // pass
            },
            else => {
// zbr:selfhost/ast_test.zbr:42
                if (!(false)) {
...
// zbr:selfhost/ast_test.zbr:56
        switch (tr) {
            .named => |nr2| {
// zbr:selfhost/ast_test.zbr:58
                std.debug.assert(std.mem.eql(u8, nr2.name, "int"));
            },
            else => {
// zbr:selfhost/ast_test.zbr:60
                if (!(false)) {
                    std.debug.print("assertion failed\n", .{});
                    unreachable;
                }
