...
pub const ResolveError = struct {
    ...
    pub fn init(message: []const u8) ResolveError {
        var self: ResolveError = undefined;
...
pub const Resolver = struct {
    ...
    errors: std.ArrayList(ResolveError) = undefined,
    symbol_count: i64 = undefined,
    pub fn init() *Resolver {
        const self = _allocator.create(Resolver) catch @panic("OOM");
        self._type_tag = _ttag_Resolver;
// zbr:selfhost\Resolver.zbr:57
        self.module_scope = std.StringHashMap(i64).init(_allocator);
// zbr:selfhost\Resolver.zbr:58
        self.class_scope = std.StringHashMap(i64).init(_allocator);
// zbr:selfhost\Resolver.zbr:59
        self.method_scope = std.StringHashMap(i64).init(_allocator);
// zbr:selfhost\Resolver.zbr:60
        self.errors = std.ArrayList(ResolveError){};
...
// zbr:selfhost\Resolver.zbr:95
                self.module_scope.put((_allocator.dupe(u8, c.name) catch @panic("OOM")), 0) catch unreachable;
// zbr:selfhost\Resolver.zbr:96
                self.symbol_count = (self.symbol_count + 1);
            },
            .struct_ => |s_ptr| {
                const s = s_ptr.*;
// zbr:selfhost\Resolver.zbr:98
                self.module_scope.put((_allocator.dupe(u8, s.name) catch @panic("OOM")), 0) catch unreachable;
// zbr:selfhost\Resolver.zbr:99
                self.symbol_count = (self.symbol_count + 1);
            },
            .union_decl => |u_ptr| {
                const u = u_ptr.*;
// zbr:selfhost\Resolver.zbr:101
                self.module_scope.put((_allocator.dupe(u8, u.name) catch @panic("OOM")), 0) catch unreachable;
// zbr:selfhost\Resolver.zbr:102
                self.symbol_count = (self.symbol_count + 1);
            },
            .sig_ => |sg_ptr| {
                const sg = sg_ptr.*;
// zbr:selfhost\Resolver.zbr:104
                self.module_scope.put((_allocator.dupe(u8, sg.name) catch @panic("OOM")), 0) catch unreachable;
...
// zbr:selfhost\Resolver.zbr:107
                self.module_scope.put((_allocator.dupe(u8, m.name) catch @panic("OOM")), 1) catch unreachable;
...
// zbr:selfhost\Resolver.zbr:110
                self.module_scope.put((_allocator.dupe(u8, u.path) catch @panic("OOM")), 5) catch unreachable;
// zbr:selfhost\Resolver.zbr:111
                for (u.exposed.items) |name| {
// zbr:selfhost\Resolver.zbr:112
                    self.module_scope.put((_allocator.dupe(u8, name) catch @panic("OOM")), 5) catch unreachable;
                }
            },
            else => {
                // pass
...
// zbr:selfhost\Resolver.zbr:119
        switch (root) {
            .module_ => |m_ptr| {
                const m = m_ptr.*;
// zbr:selfhost\Resolver.zbr:121
                for (m.decls.items) |decl| {
// zbr:selfhost\Resolver.zbr:122
...
// zbr:selfhost\Resolver.zbr:127
        switch (decl) {
            .class_ => |c_ptr| {
                const c = c_ptr.*;
// zbr:selfhost\Resolver.zbr:129
                try self.enterClass(c.name, c.members);
            },
            .struct_ => |s_ptr| {
                const s = s_ptr.*;
// zbr:selfhost\Resolver.zbr:131
                try self.enterClass(s.name, s.members);
            },
            .method_ => |m_ptr| {
                const m = m_ptr.*;
// zbr:selfhost\Resolver.zbr:133
    ...
    pub fn enterClass(self: *Resolver, name: []const u8, members: std.ArrayList(PNode)) anyerror!void {
        _ = name;
// zbr:selfhost\Resolver.zbr:141
        self.class_scope = std.StringHashMap(i64).init(_allocator);
// zbr:selfhost\Resolver.zbr:143
        for (members.items) |member| {
// zbr:selfhost\Resolver.zbr:144
            try self.collectMember(member);
        }
// zbr:selfhost\Resolver.zbr:146
        for (members.items) |member| {
...
// zbr:selfhost\Resolver.zbr:152
                self.class_scope.put((_allocator.dupe(u8, m.name) catch @panic("OOM")), 1) catch unreachable;
// zbr:selfhost\Resolver.zbr:153
                self.symbol_count = (self.symbol_count + 1);
            },
            .field_ => |f_ptr| {
                const f = f_ptr.*;
// zbr:selfhost\Resolver.zbr:155
                self.class_scope.put((_allocator.dupe(u8, f.name) catch @panic("OOM")), 2) catch unreachable;
...
// zbr:selfhost\Resolver.zbr:173
            self.method_scope.put((_allocator.dupe(u8, p.name) catch @panic("OOM")), 3) catch unreachable;
...
// zbr:selfhost\Resolver.zbr:189
                self.method_scope.put((_allocator.dupe(u8, v.name) catch @panic("OOM")), 4) catch unreachable;
...
// zbr:selfhost\Resolver.zbr:207
                    self.method_scope.put((_allocator.dupe(u8, var_name) catch @panic("OOM")), 4) catch unreachable;
                }
...
// zbr:selfhost\Resolver.zbr:225
                    self.method_scope.put((_allocator.dupe(u8, tc.catch_binding) catch @panic("OOM")), 4) catch unreachable;
                }
...
// zbr:selfhost\Resolver.zbr:232
                        self.method_scope.put((_allocator.dupe(u8, arm.binding) catch @panic("OOM")), 4) catch unreachable;
                    }
...
// zbr:selfhost\Resolver.zbr:254
                    self.errors.append(_allocator, ResolveError.init(_str_concat(_str_concat("undefined name: '", name, _allocator), "'", _allocator))) catch unreachable;
                }
            },
            .expr_binary => |b_ptr| {
                const b = b_ptr.*;
...
const _ttag_Resolver: u64 = 1513841891;
const _reflect_Resolver_name: []const u8 = "Resolver";
const _reflect_Resolver_fields: []const []const u8 = &.{"module_scope", "class_scope", "method_scope", "errors", "symbol_count"};
const _reflect_Resolver_field_types: []const []const u8 = &.{"HashMap(str, int)", "HashMap(str, int)", "HashMap(str, int)", "List(ResolveError)", "int"};
