...
// Source: test/string_methods.zbr
...
pub const Program = struct {
    ...
    pub fn main() void {
// zbr:test/string_methods.zbr:4
        const s: []const u8 = "Hello World";
// zbr:test/string_methods.zbr:5
        {
            const _pt0 = (std.ascii.allocUpperString(_allocator, s) catch unreachable);
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/string_methods.zbr:6
        {
            const _pt0 = (std.ascii.allocLowerString(_allocator, s) catch unreachable);
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/string_methods.zbr:7
        std.debug.print("{}\n", .{(if (std.mem.indexOf(u8, s, "World")) |_i| @as(i64, @intCast(_i)) else @as(i64, -1))});
// zbr:test/string_methods.zbr:8
        std.debug.print("{}\n", .{(if (std.mem.indexOf(u8, s, "xyz")) |_i| @as(i64, @intCast(_i)) else @as(i64, -1))});
// zbr:test/string_methods.zbr:9
        const r: []const u8 = (std.mem.replaceOwned(u8, _allocator, s, "World", "Zebra") catch unreachable);
        defer _allocator.free(r);
// zbr:test/string_methods.zbr:10
        std.debug.print("{s}\n", .{r});
// zbr:test/string_methods.zbr:11
        std.debug.print("{}\n", .{s.len});
// zbr:test/string_methods.zbr:12
        var _it_part = std.mem.splitSequence(u8, s, " ");
        while (_it_part.next()) |part| {
// zbr:test/string_methods.zbr:13
            std.debug.print("{s}\n", .{part});
        }
    }
