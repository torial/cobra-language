...
// Source: test/crossmod_types_test.zbr
...
pub fn _initAllocator(a: std.mem.Allocator) void {
    _allocator = a;
    @import("crossmod_types_lib.zig")._initAllocator(a);
}
...
pub const Segment = struct {
    ...
    start: crossmod_types_lib.Point = undefined,
    end_: crossmod_types_lib.Point = undefined,
    pub fn init(sx: i64, sy: i64, ex: i64, ey: i64) Segment {
        var self: Segment = undefined;
        self._type_tag = _ttag_Segment;
// zbr:test/crossmod_types_test.zbr:10
        self.start = crossmod_types_lib.Point.init(sx, sy);
// zbr:test/crossmod_types_test.zbr:11
        self.end_ = crossmod_types_lib.Point.init(ex, ey);
        return self;
    }

    pub fn label(self: *Segment) []const u8 {
// zbr:test/crossmod_types_test.zbr:14
        return (std.fmt.allocPrint(_allocator, "{s} to {s}", .{self.start.show(), self.end_.show()}) catch @panic("OOM"));
    }

};

const _ttag_Segment: u64 = 1353396102;
const _reflect_Segment_name: []const u8 = "Segment";
const _reflect_Segment_fields: []const []const u8 = &.{"start", "end_"};
const _reflect_Segment_field_types: []const []const u8 = &.{"crossmod_types_lib.Point", "crossmod_types_lib.Point"};

pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
// zbr:test/crossmod_types_test.zbr:19
        var p = crossmod_types_lib.Point.init(3, 4);
// zbr:test/crossmod_types_test.zbr:20
        std.debug.assert((p.x == 3));
// zbr:test/crossmod_types_test.zbr:21
        std.debug.assert((p.y == 4));
// zbr:test/crossmod_types_test.zbr:22
        std.debug.assert(std.mem.eql(u8, p.show(), "(3, 4)"));
// zbr:test/crossmod_types_test.zbr:24
        var s = Segment.init(0, 0, 3, 4);
// zbr:test/crossmod_types_test.zbr:25
        std.debug.assert((s.start.x == 0));
// zbr:test/crossmod_types_test.zbr:26
        std.debug.assert((s.end_.x == 3));
// zbr:test/crossmod_types_test.zbr:27
        std.debug.assert(std.mem.eql(u8, s.label(), "(0, 0) to (3, 4)"));
// zbr:test/crossmod_types_test.zbr:29
        std.debug.print("{s}\n", .{"crossmod_types_test OK"});
    }
...
pub fn main() void {
    ...
    defer _arena.deinit();
    @import("crossmod_types_lib.zig")._initAllocator(_allocator);
    Main.main();
}
