...
// Source: test/string_format_test.zbr
...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/string_format_test.zbr:6
        const n: i32 = 255;
// zbr:test/string_format_test.zbr:7
        {
            const _pt0 = (std.fmt.allocPrint(_allocator, "{x:0>8}", .{@as(u32, @bitCast(n))}) catch @panic("OOM"));
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/string_format_test.zbr:10
        const fval: f64 = 3.14159;
// zbr:test/string_format_test.zbr:11
        {
            const _pt0 = (std.fmt.allocPrint(_allocator, "{d:.2}", .{fval}) catch @panic("OOM"));
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/string_format_test.zbr:14
        const greeting: []const u8 = "hello";
// zbr:test/string_format_test.zbr:15
        {
            const _pt0 = (std.fmt.allocPrint(_allocator, "{s:>20}", .{greeting}) catch @panic("OOM"));
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/string_format_test.zbr:18
        const tag: []const u8 = "ok";
// zbr:test/string_format_test.zbr:19
        {
            const _pt0 = (std.fmt.allocPrint(_allocator, "{s:-<15}", .{tag}) catch @panic("OOM"));
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/string_format_test.zbr:22
        const word: []const u8 = "hi";
// zbr:test/string_format_test.zbr:23
        {
            const _pt0 = _pad_left(word, @as(usize, @intCast(8)), ' ', _allocator);
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/string_format_test.zbr:24
        {
            const _pt0 = _pad_right(word, @as(usize, @intCast(8)), ' ', _allocator);
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/string_format_test.zbr:25
        {
            const _pt0 = _pad_center(word, @as(usize, @intCast(8)), ' ', _allocator);
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/string_format_test.zbr:28
        const s: []const u8 = "ab";
// zbr:test/string_format_test.zbr:29
        for (s) |c| {
// zbr:test/string_format_test.zbr:30
            {
                const _pt0 = (std.fmt.allocPrint(_allocator, "{c}", .{c}) catch @panic("OOM"));
                defer _allocator.free(_pt0);
                std.debug.print("{s}\n", .{_pt0});
            }
        }
