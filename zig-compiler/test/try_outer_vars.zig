...
// zbr:test/try_outer_vars.zbr:4
        if (_zebra_lt(x, 0)) {
// zbr:test/try_outer_vars.zbr:5
            { const _rdet_3088: []const u8 = std.fmt.allocPrint(_allocator, "{}", .{x}) catch @panic("OOM");
              const _rdet_ptr_3088 = _allocator.create([]const u8) catch @panic("OOM");
              _rdet_ptr_3088.* = _rdet_3088;
              const _rshim_3088 = struct { fn call(p: *anyopaque) []const u8 {
                  return @as(*[]const u8, @alignCast(@ptrCast(p))).*;
              } };
              _error_ctx = .{ .message = "negative", .details = .{ .ptr = @ptrCast(_rdet_ptr_3088), .toString_fn = _rshim_3088.call } };
            }
            return error.ZebraError;
        }
// zbr:test/try_outer_vars.zbr:6
        return (x * 2);
    }

    pub fn main() void {
// zbr:test/try_outer_vars.zbr:9
        const base: i64 = 10;
// zbr:test/try_outer_vars.zbr:10
        const multiplier: i64 = 3;
// zbr:test/try_outer_vars.zbr:11
        var _try_err_3b0: ?anyerror = null;
        _try_blk_3b0: {
// zbr:test/try_outer_vars.zbr:12
            const result: i64 = App.risky(base) catch |_tc_267d4780138| { _try_err_3b0 = _tc_267d4780138; break :_try_blk_3b0; };
...
// zbr:test/try_outer_vars.zbr:16
            std.debug.print("{s}\n", .{"error"});
        }
// zbr:test/try_outer_vars.zbr:17
        std.debug.print("{}\n", .{base});
    }
