...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/sys_test.zbr:4
        const args = (blk: {
            const _sa_raw = std.process.argsAlloc(_allocator) catch @panic("sys.args OOM");
            var _sa_list = std.ArrayList([]const u8){};
            for (_sa_raw) |_sa_arg| _sa_list.append(_allocator, _sa_arg) catch unreachable;
            break :blk _sa_list;
        });
// zbr:test/sys_test.zbr:5
        std.debug.print("{}\n", .{@as(i64, @intCast(args.items.len))});
// zbr:test/sys_test.zbr:6
        std.debug.print("{s}\n", .{"hello stderr"});
// zbr:test/sys_test.zbr:7
        if (_zebra_gt(@as(i64, @intCast(args.items.len)), 1)) {
// zbr:test/sys_test.zbr:8
            std.debug.print("{any}\n", .{args.items[@as(usize, @intCast(1))]});
        }
    }
