...
pub const TypeChecker = struct {
    _type_tag: u64 = _ttag_TypeChecker,
    errors: std.ArrayList([]const u8) = undefined,
    _scope: *TcScope = undefined,
    pub fn init() *TypeChecker {
        const self = _allocator.create(TypeChecker) catch @panic("OOM");
        self._type_tag = _ttag_TypeChecker;
// zbr:test\typechecker.zbr:28
        self.errors = std.ArrayList([]const u8){};
// zbr:test\typechecker.zbr:29
        const s = TcScope.init();
// zbr:test\typechecker.zbr:30
        s.push(TcScopeKind.module_);
// zbr:test\typechecker.zbr:31
        self._scope = s;
        return self;
    }

    pub fn enterScope(self: *TypeChecker, kind: TcScopeKind) void {
// zbr:test\typechecker.zbr:36
        self._scope.push(kind);
    }

    pub fn leaveScope(self: *TypeChecker) void {
// zbr:test\typechecker.zbr:39
        self._scope.pop();
    }

    pub fn defineSymbol(self: *TypeChecker, name: []const u8, kind: TcSymKind, type_: TcType) bool {
// zbr:test\typechecker.zbr:42
        return self._scope.define(name, TcSymbol.init(name, kind, type_));
    }

    pub fn hasSymbol(self: *TypeChecker, name: []const u8) bool {
// zbr:test\typechecker.zbr:45
        return self._scope.hasSymbol(name);
    }

    pub fn symbolType(self: *TypeChecker, name: []const u8) TcType {
// zbr:test\typechecker.zbr:48
        if (self._scope.hasSymbol(name)) {
// zbr:test\typechecker.zbr:49
            return self._scope.lookup(name).type_;
        }
// zbr:test\typechecker.zbr:50
        return TcType{ .unknown_ = {} };
    }

    pub fn currentDepth(self: *TypeChecker) i64 {
// zbr:test\typechecker.zbr:53
        return self._scope.depth();
    }

    pub fn memberType(self: *TypeChecker, t: TcType, member: []const u8) TcType {
        _ = self;
// zbr:test\typechecker.zbr:58
        return TcStdlib.memberType(t, member);
    }

    pub fn staticCallType(self: *TypeChecker, class_: []const u8, method: []const u8) TcType {
        _ = self;
// zbr:test\typechecker.zbr:61
        return TcStdlib.staticCall(class_, method);
    }

    pub fn typeName(self: *TypeChecker, t: TcType) []const u8 {
        _ = self;
// zbr:test\typechecker.zbr:64
        return TcTypes.typeName(t);
    }

    pub fn isNumeric(self: *TypeChecker, t: TcType) bool {
        _ = self;
// zbr:test\typechecker.zbr:67
        return TcTypes.isNumeric(t);
    }

    pub fn isAssignable(self: *TypeChecker, src: TcType, dst: TcType) bool {
        _ = self;
// zbr:test\typechecker.zbr:70
        return TcTypes.isAssignable(src, dst);
    }

    pub fn checkMethod(self: *TypeChecker, ret_type: TcType, stmts: std.ArrayList(TcStmt)) void {
// zbr:test\typechecker.zbr:78
        self._scope.push(TcScopeKind.method_);
// zbr:test\typechecker.zbr:79
        const infer = TcInfer.init(self._scope);
// zbr:test\typechecker.zbr:80
        const chk = TcCheck.init(infer, ret_type);
// zbr:test\typechecker.zbr:81
        chk.checkStmts(stmts);
// zbr:test\typechecker.zbr:82
        var i: i64 = 0;
// zbr:test\typechecker.zbr:83
        while (_zebra_lt(i, @as(i64, @intCast(chk.errors.items.len)))) {
// zbr:test\typechecker.zbr:84
            self.errors.append(_allocator, chk.errors.items[@as(usize, @intCast(i))]) catch unreachable;
// zbr:test\typechecker.zbr:85
            i = (i + 1);
        }
// zbr:test\typechecker.zbr:86
        self._scope.pop();
    }

    pub fn ok(self: *TypeChecker) bool {
// zbr:test\typechecker.zbr:90
        return (@as(i64, @intCast(self.errors.items.len)) == 0);
    }

    pub fn clearErrors(self: *TypeChecker) void {
// zbr:test\typechecker.zbr:94
        var i: i64 = (@as(i64, @intCast(self.errors.items.len)) - 1);
// zbr:test\typechecker.zbr:95
        while (_zebra_ge(i, 0)) {
// zbr:test\typechecker.zbr:96
            _ = self.errors.orderedRemove(@as(usize, @intCast(i)));
// zbr:test\typechecker.zbr:97
            i = (i - 1);
        }
    }
...
const _ttag_TypeChecker: u64 = 3990440564;
const _reflect_TypeChecker_name: []const u8 = "TypeChecker";
const _reflect_TypeChecker_fields: []const []const u8 = &.{"errors", "_scope"};
const _reflect_TypeChecker_field_types: []const []const u8 = &.{"List(str)", "TcScope"};
