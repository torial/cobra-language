...
pub const Main = struct {
    pub fn main() void {
// zbr:test/sys_run_test.zbr:5
        var argv = std.ArrayList([]const u8){};
        defer argv.deinit(_allocator);
// zbr:test/sys_run_test.zbr:6
        argv.append(_allocator, "cmd") catch unreachable;
// zbr:test/sys_run_test.zbr:7
        argv.append(_allocator, "/c") catch unreachable;
// zbr:test/sys_run_test.zbr:8
        argv.append(_allocator, "echo hello") catch unreachable;
// zbr:test/sys_run_test.zbr:9
        const result: SysRunResult = _sys_run(argv);
// zbr:test/sys_run_test.zbr:10
        std.debug.print("{}\n", .{(result.exit_code == 0)});
// zbr:test/sys_run_test.zbr:11
        std.debug.print("{}\n", .{(std.mem.indexOf(u8, result.stdout, "hello") != null)});
// zbr:test/sys_run_test.zbr:12
        std.debug.print("{}\n", .{(result.stderr.len == 0)});
// zbr:test/sys_run_test.zbr:15
        var bad = std.ArrayList([]const u8){};
        defer bad.deinit(_allocator);
// zbr:test/sys_run_test.zbr:16
        bad.append(_allocator, "cmd") catch unreachable;
// zbr:test/sys_run_test.zbr:17
        bad.append(_allocator, "/c") catch unreachable;
// zbr:test/sys_run_test.zbr:18
        bad.append(_allocator, "exit 42") catch unreachable;
// zbr:test/sys_run_test.zbr:19
        const bad_result: SysRunResult = _sys_run(bad);
// zbr:test/sys_run_test.zbr:20
        std.debug.print("{}\n", .{(bad_result.exit_code == 42)});
    }
