...
// Source: test/try_catch_full.zbr
...
// zbr:test/try_catch_full.zbr:4
        if (_zebra_lt(x, 0)) {
// zbr:test/try_catch_full.zbr:5
            { const _rdet_2a88: []const u8 = std.fmt.allocPrint(_allocator, "{}", .{x}) catch @panic("OOM");
              const _rdet_ptr_2a88 = _allocator.create([]const u8) catch @panic("OOM");
              _rdet_ptr_2a88.* = _rdet_2a88;
              const _rshim_2a88 = struct { fn call(p: *anyopaque) []const u8 {
                  return @as(*[]const u8, @alignCast(@ptrCast(p))).*;
              } };
              _error_ctx = .{ .message = "negative input", .details = .{ .ptr = @ptrCast(_rdet_ptr_2a88), .toString_fn = _rshim_2a88.call } };
            }
            return error.ZebraError;
        }
// zbr:test/try_catch_full.zbr:6
        return (x * 2);
    }

    pub fn main() void {
// zbr:test/try_catch_full.zbr:9
        var _try_err_2948: ?anyerror = null;
        _try_blk_2948: {
// zbr:test/try_catch_full.zbr:10
            const result: i64 = App.risky(5) catch |_tc_1ee0f3a0378| { _try_err_2948 = _tc_1ee0f3a0378; break :_try_blk_2948; };
// zbr:test/try_catch_full.zbr:11
            std.debug.print("{}\n", .{result});
            break :_try_blk_2948;
        }
        if (_try_err_2948 != null) {
// zbr:test/try_catch_full.zbr:13
            std.debug.print("{s}\n", .{"caught an error"});
        }
// zbr:test/try_catch_full.zbr:15
        std.debug.print("{s}\n", .{"after try"});
    }
