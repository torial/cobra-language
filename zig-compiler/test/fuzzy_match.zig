...
pub const Program = struct {
    ...
    pub fn buildNgrams(text: []const u8) std.StringHashMap(i64) {
// zbr:test/fuzzy_match.zbr:9
...
// zbr:test/fuzzy_match.zbr:12
            clist.append(_allocator, (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(c, &_cpbuf) catch 1; const _cpout = _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); break :blk @as([]const u8, _cpout); })) catch unreachable;
        }
// zbr:test/fuzzy_match.zbr:13
        var i: i64 = 0;
// zbr:test/fuzzy_match.zbr:14
        while (_zebra_lt(i, (@as(i64, @intCast(clist.items.len)) - 2))) {
// zbr:test/fuzzy_match.zbr:15
            const tri = (std.mem.concat(_allocator, u8, &.{ (std.mem.concat(_allocator, u8, &.{ clist.items[@as(usize, @intCast(i))], clist.items[@as(usize, @intCast((i + 1)))] }) catch unreachable), clist.items[@as(usize, @intCast((i + 2)))] }) catch unreachable);
            defer _allocator.free(tri);
// zbr:test/fuzzy_match.zbr:16
            if (ngrams.contains(tri)) {
// zbr:test/fuzzy_match.zbr:17
                ngrams.put((_allocator.dupe(u8, tri) catch @panic("OOM")), ((ngrams.get(tri) orelse undefined) + 1)) catch unreachable;
            } else {
// zbr:test/fuzzy_match.zbr:19
                ngrams.put((_allocator.dupe(u8, tri) catch @panic("OOM")), 1) catch unreachable;
            }
// zbr:test/fuzzy_match.zbr:20
            i = (i + 1);
        }
// zbr:test/fuzzy_match.zbr:21
        return ngrams;
    }

    pub fn intersection(a: std.StringHashMap(i64), b: std.StringHashMap(i64)) i64 {
// zbr:test/fuzzy_match.zbr:24
        var total: i64 = 0;
// zbr:test/fuzzy_match.zbr:25
        var _it_tri = a.iterator();
        while (_it_tri.next()) |_e_tri| {
            const tri = _e_tri.key_ptr.*;
            const ca = _e_tri.value_ptr.*;
// zbr:test/fuzzy_match.zbr:26
            if (b.contains(tri)) {
// zbr:test/fuzzy_match.zbr:27
                const cb = (b.get(tri) orelse undefined);
// zbr:test/fuzzy_match.zbr:28
                if (_zebra_lt(ca, cb)) {
// zbr:test/fuzzy_match.zbr:29
                    total = (total + ca);
                } else {
// zbr:test/fuzzy_match.zbr:31
                    total = (total + cb);
                }
            }
        }
// zbr:test/fuzzy_match.zbr:32
        return total;
    }

    pub fn loadVerses(path: []const u8) std.ArrayList([]const u8) {
...
// zbr:test/fuzzy_match.zbr:39
            if (first) {
// zbr:test/fuzzy_match.zbr:40
                first = false;
            } else {
...
// zbr:test/fuzzy_match.zbr:43
                ...
                while (_it_p.next()) |p| {
// zbr:test/fuzzy_match.zbr:44
                    parts.append(_allocator, p) catch unreachable;
                }
// zbr:test/fuzzy_match.zbr:45
                if (_zebra_ge(@as(i64, @intCast(parts.items.len)), 3)) {
// zbr:test/fuzzy_match.zbr:46
                    const text: []const u8 = parts.items[@as(usize, @intCast(2))];
// zbr:test/fuzzy_match.zbr:47
                    if (_zebra_gt(@as(i64, @intCast(std.unicode.utf8CountCodepoints(text) catch 0)), 4)) {
// zbr:test/fuzzy_match.zbr:48
                        verses.append(_allocator, text) catch unreachable;
                    }
                }
            }
        }
// zbr:test/fuzzy_match.zbr:49
        return verses;
    }
...
// zbr:test/fuzzy_match.zbr:55
        {
            const _pt0 = (std.fmt.allocPrint(_allocator, "Mat: {} verses", .{@as(i64, @intCast(mat_verses.items.len))}) catch @panic("OOM"));
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
...
// zbr:test/fuzzy_match.zbr:62
        while (_zebra_lt(pi, @as(i64, @intCast(mat_verses.items.len)))) {
// zbr:test/fuzzy_match.zbr:63
            const ptext: []const u8 = mat_verses.items[@as(usize, @intCast(pi))];
// zbr:test/fuzzy_match.zbr:64
            const ptotal = (@as(i64, @intCast(std.unicode.utf8CountCodepoints(ptext) catch 0)) - 2);
// zbr:test/fuzzy_match.zbr:65
            if (_zebra_gt(ptotal, 3)) {
// zbr:test/fuzzy_match.zbr:66
                mat_ngrams.append(_allocator, Program.buildNgrams(ptext)) catch unreachable;
// zbr:test/fuzzy_match.zbr:67
                mat_totals.append(_allocator, ptotal) catch unreachable;
// zbr:test/fuzzy_match.zbr:68
                mat_idx.append(_allocator, pi) catch unreachable;
            }
// zbr:test/fuzzy_match.zbr:69
            pi = (pi + 1);
        }
// zbr:test/fuzzy_match.zbr:70
        {
            const _pt0 = (std.fmt.allocPrint(_allocator, "Mat ngrams cached: {}", .{@as(i64, @intCast(mat_ngrams.items.len))}) catch @panic("OOM"));
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/fuzzy_match.zbr:72
        var mi: i64 = 0;
// zbr:test/fuzzy_match.zbr:73
        while (_zebra_lt(mi, @as(i64, @intCast(mark_verses.items.len)))) {
// zbr:test/fuzzy_match.zbr:74
            const mtext: []const u8 = mark_verses.items[@as(usize, @intCast(mi))];
// zbr:test/fuzzy_match.zbr:75
            const total_m = (@as(i64, @intCast(std.unicode.utf8CountCodepoints(mtext) catch 0)) - 2);
...
// zbr:test/fuzzy_match.zbr:79
                while (_zebra_lt(ti, @as(i64, @intCast(mat_ngrams.items.len)))) {
// zbr:test/fuzzy_match.zbr:80
                    const total_t: i64 = mat_totals.items[@as(usize, @intCast(ti))];
// zbr:test/fuzzy_match.zbr:81
                    const ngrams_t: std.StringHashMap(i64) = mat_ngrams.items[@as(usize, @intCast(ti))];
// zbr:test/fuzzy_match.zbr:82
                    const inter = Program.intersection(ngrams_m, ngrams_t);
// zbr:test/fuzzy_match.zbr:84
                    if (_zebra_ge((20 * inter), (7 * (total_m + total_t)))) {
// zbr:test/fuzzy_match.zbr:85
                        const orig_ti: i64 = mat_idx.items[@as(usize, @intCast(ti))];
// zbr:test/fuzzy_match.zbr:86
                        {
                            const _pt0 = (std.fmt.allocPrint(_allocator, "MAR[{}] ~ MAT[{}] inter={} total={}", .{mi, orig_ti, inter, (total_m + total_t)}) catch @panic("OOM"));
                            defer _allocator.free(_pt0);
                            std.debug.print("{s}\n", .{_pt0});
                        }
                    }
// zbr:test/fuzzy_match.zbr:87
                    ti = (ti + 1);
                }
            }
// zbr:test/fuzzy_match.zbr:88
            mi = (mi + 1);
        }
    }
