...
pub const TcSymbol = struct {
    name: []const u8 = undefined,
    kind: TcSymKind = undefined,
    type_: TcType = undefined,
    pub fn init(n: []const u8, k: TcSymKind, t: TcType) TcSymbol {
        var self: TcSymbol = undefined;
// zbr:test\tc_scope.zbr:58
        self.name = n;
// zbr:test\tc_scope.zbr:59
        self.kind = k;
// zbr:test\tc_scope.zbr:60
        self.type_ = t;
        return self;
    }
...
pub const TcScope = struct {
    _type_tag: u64 = _ttag_TcScope,
    _names: std.ArrayList([]const u8) = undefined,
    _syms: std.ArrayList(TcSymbol) = undefined,
    _limits: std.ArrayList(i64) = undefined,
    _kinds: std.ArrayList(TcScopeKind) = undefined,
    pub fn init() *TcScope {
        const self = _allocator.create(TcScope) catch @panic("OOM");
        self._type_tag = _ttag_TcScope;
// zbr:test\tc_scope.zbr:72
        self._names = std.ArrayList([]const u8){};
// zbr:test\tc_scope.zbr:73
        self._syms = std.ArrayList(TcSymbol){};
// zbr:test\tc_scope.zbr:74
        self._limits = std.ArrayList(i64){};
// zbr:test\tc_scope.zbr:75
        self._kinds = std.ArrayList(TcScopeKind){};
        return self;
    }

    pub fn push(self: *TcScope, kind: TcScopeKind) void {
// zbr:test\tc_scope.zbr:79
        self._limits.append(_allocator, @as(i64, @intCast(self._names.items.len))) catch unreachable;
// zbr:test\tc_scope.zbr:80
        self._kinds.append(_allocator, kind) catch unreachable;
    }

    pub fn pop(self: *TcScope) void {
// zbr:test\tc_scope.zbr:84
        const frame_start = self._limits.items[@as(usize, @intCast((@as(i64, @intCast(self._limits.items.len)) - 1)))];
// zbr:test\tc_scope.zbr:85
        while (_zebra_gt(@as(i64, @intCast(self._names.items.len)), frame_start)) {
// zbr:test\tc_scope.zbr:86
            _ = self._names.orderedRemove(@as(usize, @intCast((@as(i64, @intCast(self._names.items.len)) - 1))));
// zbr:test\tc_scope.zbr:87
            _ = self._syms.orderedRemove(@as(usize, @intCast((@as(i64, @intCast(self._syms.items.len)) - 1))));
        }
// zbr:test\tc_scope.zbr:88
        _ = self._limits.orderedRemove(@as(usize, @intCast((@as(i64, @intCast(self._limits.items.len)) - 1))));
// zbr:test\tc_scope.zbr:89
        _ = self._kinds.orderedRemove(@as(usize, @intCast((@as(i64, @intCast(self._kinds.items.len)) - 1))));
    }

    pub fn depth(self: *TcScope) i64 {
// zbr:test\tc_scope.zbr:93
        return @as(i64, @intCast(self._limits.items.len));
    }

    pub fn currentKind(self: *TcScope) TcScopeKind {
// zbr:test\tc_scope.zbr:97
        return self._kinds.items[@as(usize, @intCast((@as(i64, @intCast(self._kinds.items.len)) - 1)))];
    }

    pub fn define(self: *TcScope, name: []const u8, sym: TcSymbol) bool {
// zbr:test\tc_scope.zbr:102
        const frame_start = self._limits.items[@as(usize, @intCast((@as(i64, @intCast(self._limits.items.len)) - 1)))];
// zbr:test\tc_scope.zbr:103
        var i: i64 = (@as(i64, @intCast(self._names.items.len)) - 1);
// zbr:test\tc_scope.zbr:104
        while (_zebra_ge(i, frame_start)) {
// zbr:test\tc_scope.zbr:105
            if (std.mem.eql(u8, self._names.items[@as(usize, @intCast(i))], name)) {
// zbr:test\tc_scope.zbr:106
                return false;
            }
// zbr:test\tc_scope.zbr:107
            i = (i - 1);
        }
// zbr:test\tc_scope.zbr:108
        self._names.append(_allocator, name) catch unreachable;
// zbr:test\tc_scope.zbr:109
        self._syms.append(_allocator, sym) catch unreachable;
// zbr:test\tc_scope.zbr:110
        return true;
    }

    pub fn hasLocal(self: *TcScope, name: []const u8) bool {
// zbr:test\tc_scope.zbr:114
        const frame_start = self._limits.items[@as(usize, @intCast((@as(i64, @intCast(self._limits.items.len)) - 1)))];
// zbr:test\tc_scope.zbr:115
        var i: i64 = (@as(i64, @intCast(self._names.items.len)) - 1);
// zbr:test\tc_scope.zbr:116
        while (_zebra_ge(i, frame_start)) {
// zbr:test\tc_scope.zbr:117
            if (std.mem.eql(u8, self._names.items[@as(usize, @intCast(i))], name)) {
// zbr:test\tc_scope.zbr:118
                return true;
            }
// zbr:test\tc_scope.zbr:119
            i = (i - 1);
        }
// zbr:test\tc_scope.zbr:120
        return false;
    }

    pub fn getLocal(self: *TcScope, name: []const u8) TcSymbol {
// zbr:test\tc_scope.zbr:125
        const frame_start = self._limits.items[@as(usize, @intCast((@as(i64, @intCast(self._limits.items.len)) - 1)))];
// zbr:test\tc_scope.zbr:126
        var i: i64 = (@as(i64, @intCast(self._names.items.len)) - 1);
// zbr:test\tc_scope.zbr:127
        while (_zebra_ge(i, frame_start)) {
// zbr:test\tc_scope.zbr:128
            if (std.mem.eql(u8, self._names.items[@as(usize, @intCast(i))], name)) {
// zbr:test\tc_scope.zbr:129
                return self._syms.items[@as(usize, @intCast(i))];
            }
// zbr:test\tc_scope.zbr:130
            i = (i - 1);
        }
// zbr:test\tc_scope.zbr:131
        return TcSymbol.init("", TcSymKind.local, TcType{ .unknown_ = {} });
    }

    pub fn hasSymbol(self: *TcScope, name: []const u8) bool {
// zbr:test\tc_scope.zbr:135
        var i: i64 = (@as(i64, @intCast(self._names.items.len)) - 1);
// zbr:test\tc_scope.zbr:136
        while (_zebra_ge(i, 0)) {
// zbr:test\tc_scope.zbr:137
            if (std.mem.eql(u8, self._names.items[@as(usize, @intCast(i))], name)) {
// zbr:test\tc_scope.zbr:138
                return true;
            }
// zbr:test\tc_scope.zbr:139
            i = (i - 1);
        }
// zbr:test\tc_scope.zbr:140
        return false;
    }

    pub fn lookup(self: *TcScope, name: []const u8) TcSymbol {
// zbr:test\tc_scope.zbr:145
        var i: i64 = (@as(i64, @intCast(self._names.items.len)) - 1);
// zbr:test\tc_scope.zbr:146
        while (_zebra_ge(i, 0)) {
// zbr:test\tc_scope.zbr:147
            if (std.mem.eql(u8, self._names.items[@as(usize, @intCast(i))], name)) {
// zbr:test\tc_scope.zbr:148
                return self._syms.items[@as(usize, @intCast(i))];
            }
// zbr:test\tc_scope.zbr:149
            i = (i - 1);
        }
// zbr:test\tc_scope.zbr:150
        return TcSymbol.init("", TcSymKind.local, TcType{ .unknown_ = {} });
    }

};

const _ttag_TcScope: u64 = 726974436;
const _reflect_TcScope_name: []const u8 = "TcScope";
const _reflect_TcScope_fields: []const []const u8 = &.{"_names", "_syms", "_limits", "_kinds"};
const _reflect_TcScope_field_types: []const []const u8 = &.{"List(str)", "List(TcSymbol)", "List(int)", "List(TcScopeKind)"};
