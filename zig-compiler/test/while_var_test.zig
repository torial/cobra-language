...
// Source: test/while_var_test.zbr
...
pub const WhileVarTest = struct {
    _type_tag: u64 = _ttag_WhileVarTest,
    pub fn testScanAlpha(self: *WhileVarTest) []const u8 {
        _ = self;
// zbr:test/while_var_test.zbr:3
        const src = "hello123";
// zbr:test/while_var_test.zbr:4
        var pos: i64 = 0;
// zbr:test/while_var_test.zbr:5
        var word: []const u8 = "";
// zbr:test/while_var_test.zbr:6
        while (true) {
            const c = src[@intCast(pos)];
            if (!((_zebra_ge(c, 'a') and _zebra_le(c, 'z')))) break;
// zbr:test/while_var_test.zbr:7
            word = _str_concat(word, src[@intCast(pos)..@intCast((pos + 1))], _allocator);
// zbr:test/while_var_test.zbr:8
            pos = (pos + 1);
        }
// zbr:test/while_var_test.zbr:9
        return word;
    }

    pub fn testScanDigits(self: *WhileVarTest) []const u8 {
        _ = self;
// zbr:test/while_var_test.zbr:12
        const src = "42abc";
// zbr:test/while_var_test.zbr:13
        var pos: i64 = 0;
// zbr:test/while_var_test.zbr:14
        var digits: []const u8 = "";
// zbr:test/while_var_test.zbr:15
        while (true) {
            const c = src[@intCast(pos)];
            if (!((_zebra_ge(c, '0') and _zebra_le(c, '9')))) break;
// zbr:test/while_var_test.zbr:16
            digits = _str_concat(digits, src[@intCast(pos)..@intCast((pos + 1))], _allocator);
// zbr:test/while_var_test.zbr:17
            pos = (pos + 1);
        }
// zbr:test/while_var_test.zbr:18
        return digits;
    }

    pub fn testEmpty(self: *WhileVarTest) i64 {
        _ = self;
// zbr:test/while_var_test.zbr:21
        const src = "!xyz";
// zbr:test/while_var_test.zbr:22
        var pos: i64 = 0;
// zbr:test/while_var_test.zbr:23
        while (true) {
            const c = src[@intCast(pos)];
            if (!((_zebra_ge(c, 'a') and _zebra_le(c, 'z')))) break;
// zbr:test/while_var_test.zbr:24
            pos = (pos + 1);
        }
// zbr:test/while_var_test.zbr:25
        return pos;
    }

    pub fn init() WhileVarTest {
        var self: WhileVarTest = undefined;
        self._type_tag = _ttag_WhileVarTest;
        return self;
    }

};

const _ttag_WhileVarTest: u64 = 1916187439;
const _reflect_WhileVarTest_name: []const u8 = "WhileVarTest";
const _reflect_WhileVarTest_fields: []const []const u8 = &.{};
const _reflect_WhileVarTest_field_types: []const []const u8 = &.{};

pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
// zbr:test/while_var_test.zbr:29
        var t = WhileVarTest.init();
// zbr:test/while_var_test.zbr:30
        std.debug.assert(std.mem.eql(u8, t.testScanAlpha(), "hello"));
// zbr:test/while_var_test.zbr:31
        std.debug.assert(std.mem.eql(u8, t.testScanDigits(), "42"));
// zbr:test/while_var_test.zbr:32
        std.debug.assert((t.testEmpty() == 0));
// zbr:test/while_var_test.zbr:33
        std.debug.print("{s}\n", .{"while_var_test: all OK"});
    }
