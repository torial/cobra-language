...
pub const Main = struct {
    ...
    pub fn testLocalhostResolve() void {
// zbr:test/dns_test.zbr:5
        std.debug.print("{s}\n", .{"Resolving localhost:"});
// zbr:test/dns_test.zbr:6
        for (_net_resolve("localhost")) |ip| {
// zbr:test/dns_test.zbr:7
            std.debug.print("{s}\n", .{ip});
        }
    }
...
// zbr:test/dns_test.zbr:12
        hosts.append(_allocator, "127.0.0.1") catch unreachable;
// zbr:test/dns_test.zbr:13
        for (hosts.items) |hostname| {
// zbr:test/dns_test.zbr:14
            {
                const _pt0 = (std.mem.concat(_allocator, u8, &.{ "Resolving: ", hostname }) catch unreachable);
                defer _allocator.free(_pt0);
                std.debug.print("{s}\n", .{_pt0});
            }
// zbr:test/dns_test.zbr:15
            for (_net_resolve(hostname)) |ip| {
// zbr:test/dns_test.zbr:16
                {
                    const _pt0 = (std.mem.concat(_allocator, u8, &.{ "  -> ", ip }) catch unreachable);
                    defer _allocator.free(_pt0);
                    std.debug.print("{s}\n", .{_pt0});
                }
            }
...
// zbr:test/dns_test.zbr:20
        std.debug.print("{s}\n", .{"Attempting to resolve invalid host:"});
// zbr:test/dns_test.zbr:21
        const results = _net_resolve("invalid.hostname.that.does.not.exist.local");
// zbr:test/dns_test.zbr:22
        {
            const _pt0 = (std.mem.concat(_allocator, u8, &.{ (std.mem.concat(_allocator, u8, &.{ "Got ", (std.fmt.allocPrint(_allocator, "{}", .{@as(i64, @intCast(results.len))}) catch unreachable) }) catch unreachable), " results" }) catch unreachable);
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
    }
    ...
    pub fn main() void {
// zbr:test/dns_test.zbr:25
        Main.testLocalhostResolve();
// zbr:test/dns_test.zbr:26
        Main.testMultipleHosts();
// zbr:test/dns_test.zbr:27
        Main.testInvalidHost();
    }
