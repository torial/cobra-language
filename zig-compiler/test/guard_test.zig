...
pub const Validator = struct {
    _type_tag: u64 = _ttag_Validator,
    pub fn checkPositive(n: i64) anyerror!i64 {
// zbr:test/guard_test.zbr:5
        if (!(_zebra_gt(n, 0))) {
// zbr:test/guard_test.zbr:6
            _error_ctx = .{ .message = "expected positive number", .details = null };
            return error.ZebraError;
        }
// zbr:test/guard_test.zbr:7
        return n;
    }

    pub fn firstChar(s: []const u8) []const u8 {
// zbr:test/guard_test.zbr:11
        if (!(_zebra_gt(s.len, 0))) {
// zbr:test/guard_test.zbr:11
            return "?";
        }
// zbr:test/guard_test.zbr:12
        return s[@intCast(0)..@intCast(1)];
    }

    pub fn init() Validator {
        var self: Validator = undefined;
        self._type_tag = _ttag_Validator;
        return self;
    }

};

const _ttag_Validator: u64 = 379857105;
const _reflect_Validator_name: []const u8 = "Validator";
const _reflect_Validator_fields: []const []const u8 = &.{};
const _reflect_Validator_field_types: []const []const u8 = &.{};

pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() anyerror!void {
// zbr:test/guard_test.zbr:18
        const x = try Validator.checkPositive(42);
// zbr:test/guard_test.zbr:19
        std.debug.print("{}\n", .{x});
// zbr:test/guard_test.zbr:22
        const c = Validator.firstChar("hello");
// zbr:test/guard_test.zbr:23
        std.debug.print("{s}\n", .{c});
// zbr:test/guard_test.zbr:25
        const empty = Validator.firstChar("");
// zbr:test/guard_test.zbr:26
        std.debug.print("{s}\n", .{empty});
// zbr:test/guard_test.zbr:29
        const bad = try Validator.checkPositive((-5));
// zbr:test/guard_test.zbr:30
        std.debug.print("{}\n", .{bad});
    }
