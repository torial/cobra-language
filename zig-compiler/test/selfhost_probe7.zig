...
// Source: test/selfhost_probe7.zbr
...
// zbr:test/selfhost_probe7.zbr:16
        self.buf = std.ArrayList(u8){};
        return self;
    }

    pub fn append(self: *Writer, s: []const u8) void {
// zbr:test/selfhost_probe7.zbr:19
        self.buf.appendSlice(_allocator, s) catch @panic("OOM");
    }

    pub fn result(self: *Writer) []const u8 {
// zbr:test/selfhost_probe7.zbr:22
        return self.buf.toOwnedSlice(_allocator) catch @panic("OOM");
    }
...
const _reflect_Writer_fields: []const []const u8 = &.{"buf"};
const _reflect_Writer_field_types: []const []const u8 = &.{"StringBuilder"};

pub const SharedState = struct {
    _type_tag: u64 = _ttag_SharedState,
    count: i64 = undefined,
    pub fn init() *SharedState {
        const self = _allocator.create(SharedState) catch @panic("OOM");
        self._type_tag = _ttag_SharedState;
// zbr:test/selfhost_probe7.zbr:28
        self.count = 0;
        return self;
    }

    pub fn increment(self: *SharedState) void {
// zbr:test/selfhost_probe7.zbr:31
        self.count += 1;
    }

};

const _ttag_SharedState: u64 = 2425144425;
const _reflect_SharedState_name: []const u8 = "SharedState";
const _reflect_SharedState_fields: []const []const u8 = &.{"count"};
const _reflect_SharedState_field_types: []const []const u8 = &.{"int"};
...
pub const Generator = struct {
    ...
    try_label: ?[]const u8 = undefined,
    catch_var: []const u8 = undefined,
    source: []const u8 = undefined,
    pub fn init(w: *Writer, state: *SharedState, owner: []const u8, indent: i64, in_method: bool, is_struct: bool, try_label: ?[]const u8, catch_var: []const u8, source: []const u8) Generator {
        var self: Generator = undefined;
// zbr:test/selfhost_probe7.zbr:45
        self.w = w;
// zbr:test/selfhost_probe7.zbr:46
        self.state = state;
// zbr:test/selfhost_probe7.zbr:47
        self.owner = owner;
// zbr:test/selfhost_probe7.zbr:48
        self.indent = indent;
// zbr:test/selfhost_probe7.zbr:49
        self.in_method = in_method;
// zbr:test/selfhost_probe7.zbr:50
        self.is_struct = is_struct;
// zbr:test/selfhost_probe7.zbr:51
        self.try_label = try_label;
// zbr:test/selfhost_probe7.zbr:52
        self.catch_var = catch_var;
// zbr:test/selfhost_probe7.zbr:53
        self.source = source;
        return self;
    }

    pub fn withOwner(self: *Generator, new_owner: []const u8) Generator {
// zbr:test/selfhost_probe7.zbr:56
        const g = blk: {
            var _tmp = self.*;
            _tmp.owner = new_owner;
            break :blk _tmp;
        };
// zbr:test/selfhost_probe7.zbr:58
        return g;
    }

    pub fn withIndent(self: *Generator) Generator {
// zbr:test/selfhost_probe7.zbr:61
        const g = blk: {
            var _tmp = self.*;
            _tmp.indent = (self.indent + 1);
            break :blk _tmp;
        };
// zbr:test/selfhost_probe7.zbr:63
        return g;
    }

    pub fn withMethod(self: *Generator) Generator {
// zbr:test/selfhost_probe7.zbr:66
        const g = blk: {
            var _tmp = self.*;
            _tmp.in_method = true;
            break :blk _tmp;
        };
// zbr:test/selfhost_probe7.zbr:68
        return g;
    }

    pub fn withTryLabel(self: *Generator, label: []const u8, err_var: []const u8) Generator {
// zbr:test/selfhost_probe7.zbr:71
        const g = blk: {
            var _tmp = self.*;
            _tmp.try_label = label;
            _tmp.catch_var = err_var;
            break :blk _tmp;
        };
// zbr:test/selfhost_probe7.zbr:74
        return g;
    }

    pub fn emit(self: *Generator, s: []const u8) void {
// zbr:test/selfhost_probe7.zbr:77
        self.w.append(s);
    }

    pub fn emitIndented(self: *Generator, s: []const u8) void {
// zbr:test/selfhost_probe7.zbr:80
        var i: i64 = 0;
// zbr:test/selfhost_probe7.zbr:81
        while (_zebra_lt(i, self.indent)) {
// zbr:test/selfhost_probe7.zbr:82
            self.w.append("    ");
// zbr:test/selfhost_probe7.zbr:83
            i += 1;
        }
// zbr:test/selfhost_probe7.zbr:84
        self.w.append(s);
// zbr:test/selfhost_probe7.zbr:85
        self.w.append("\n");
    }
...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/selfhost_probe7.zbr:90
        Main.test1SharedClassViaStructCopy();
// zbr:test/selfhost_probe7.zbr:91
        Main.test2ContextForkingWithSharedWriter();
// zbr:test/selfhost_probe7.zbr:92
        Main.test3OptionalStrField();
// zbr:test/selfhost_probe7.zbr:93
        Main.test4NestedIndentation();
// zbr:test/selfhost_probe7.zbr:94
        std.debug.print("{s}\n", .{"selfhost_probe7: all OK"});
    }

    pub fn test1SharedClassViaStructCopy() void {
// zbr:test/selfhost_probe7.zbr:97
        const st = SharedState.init();
// zbr:test/selfhost_probe7.zbr:98
        const wr = Writer.init();
// zbr:test/selfhost_probe7.zbr:99
        var g1 = Generator.init(wr, st, "", 0, false, false, null, "", "");
// zbr:test/selfhost_probe7.zbr:100
        const g2 = g1.withOwner("Foo");
// zbr:test/selfhost_probe7.zbr:101
        const g3 = g1.withOwner("Bar");
// zbr:test/selfhost_probe7.zbr:104
        g1.state.increment();
// zbr:test/selfhost_probe7.zbr:105
        g2.state.increment();
// zbr:test/selfhost_probe7.zbr:106
        g3.state.increment();
// zbr:test/selfhost_probe7.zbr:108
        if (!((g1.state.count == 3))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/selfhost_probe7.zbr:109
        if (!((g2.state.count == 3))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/selfhost_probe7.zbr:110
        if (!((g3.state.count == 3))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/selfhost_probe7.zbr:111
        std.debug.print("{s}\n", .{"test1 OK"});
    }

    pub fn test2ContextForkingWithSharedWriter() void {
// zbr:test/selfhost_probe7.zbr:114
        const wr = Writer.init();
// zbr:test/selfhost_probe7.zbr:115
        const st = SharedState.init();
// zbr:test/selfhost_probe7.zbr:116
        var base = Generator.init(wr, st, "", 0, false, false, null, "", "src.zbr");
// zbr:test/selfhost_probe7.zbr:117
        var g1 = base.withOwner("MyClass");
// zbr:test/selfhost_probe7.zbr:118
        var g2 = g1.withMethod();
// zbr:test/selfhost_probe7.zbr:119
        var g3a = g2.withIndent();
// zbr:test/selfhost_probe7.zbr:120
        var g3 = g3a.withIndent();
// zbr:test/selfhost_probe7.zbr:123
        base.emit("// module\n");
// zbr:test/selfhost_probe7.zbr:124
        g1.emit("pub const MyClass = struct {\n");
// zbr:test/selfhost_probe7.zbr:125
        g3.emitIndented("x: i64 = 0,");
// zbr:test/selfhost_probe7.zbr:126
        g1.emit("};\n");
// zbr:test/selfhost_probe7.zbr:128
        const out = wr.result();
// zbr:test/selfhost_probe7.zbr:129
        if (!(std.mem.eql(u8, out, "// module\npub const MyClass = struct {\n        x: i64 = 0,\n};\n"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/selfhost_probe7.zbr:130
        std.debug.print("{s}\n", .{"test2 OK"});
    }

    pub fn test3OptionalStrField() void {
// zbr:test/selfhost_probe7.zbr:133
        const wr = Writer.init();
// zbr:test/selfhost_probe7.zbr:134
        const st = SharedState.init();
// zbr:test/selfhost_probe7.zbr:135
        var g = Generator.init(wr, st, "", 0, false, false, null, "", "");
// zbr:test/selfhost_probe7.zbr:137
        if (!((g.try_label == null))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/selfhost_probe7.zbr:138
        if (!(std.mem.eql(u8, g.catch_var, ""))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/selfhost_probe7.zbr:140
        const g2 = g.withTryLabel("blk_try", "_err");
// zbr:test/selfhost_probe7.zbr:141
        if (!(std.mem.eql(u8, g2.try_label.?, "blk_try"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/selfhost_probe7.zbr:142
        if (!(std.mem.eql(u8, g2.catch_var, "_err"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/selfhost_probe7.zbr:145
        if (!((g.try_label == null))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/selfhost_probe7.zbr:146
        std.debug.print("{s}\n", .{"test3 OK"});
    }

    pub fn test4NestedIndentation() void {
// zbr:test/selfhost_probe7.zbr:149
        const wr = Writer.init();
// zbr:test/selfhost_probe7.zbr:150
        const st = SharedState.init();
// zbr:test/selfhost_probe7.zbr:151
        var g = Generator.init(wr, st, "Outer", 0, false, false, null, "", "");
// zbr:test/selfhost_probe7.zbr:153
        var g1 = g.withIndent();
// zbr:test/selfhost_probe7.zbr:154
        var g2a = g1.withOwner("Inner");
// zbr:test/selfhost_probe7.zbr:155
        var g2 = g2a.withIndent();
// zbr:test/selfhost_probe7.zbr:157
        std.debug.assert((g.indent == 0));
// zbr:test/selfhost_probe7.zbr:158
        std.debug.assert((g1.indent == 1));
// zbr:test/selfhost_probe7.zbr:159
        std.debug.assert((g2.indent == 2));
// zbr:test/selfhost_probe7.zbr:161
        g.emitIndented("level0");
// zbr:test/selfhost_probe7.zbr:162
        g1.emitIndented("level1");
// zbr:test/selfhost_probe7.zbr:163
        g2.emitIndented("level2");
// zbr:test/selfhost_probe7.zbr:165
        const out = wr.result();
// zbr:test/selfhost_probe7.zbr:166
        if (!(std.mem.eql(u8, out, "level0\n    level1\n        level2\n"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/selfhost_probe7.zbr:167
        std.debug.print("{s}\n", .{"test4 OK"});
    }
