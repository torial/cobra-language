...
pub const Math = struct {
    _type_id: u32 = _tid_Math,
    pub fn minMax(self: *Math, a: i64, b: i64) struct { i64, i64 } {
        _ = self;
// zbr:test/tuple_test.zbr:3
        if (_zebra_lt(a, b)) {
// zbr:test/tuple_test.zbr:4
            return .{ @as(i64, a), @as(i64, b) };
        } else {
// zbr:test/tuple_test.zbr:6
            return .{ @as(i64, b), @as(i64, a) };
        }
    }

    pub fn init() Math {
        var self: Math = undefined;
        self._type_id = _tid_Math;
        return self;
    }

};

const _tid_Math: u32 = 1432994735;
...
pub const Program = struct {
    ...
    pub fn main() void {
// zbr:test/tuple_test.zbr:10
        var m = Math.init();
// zbr:test/tuple_test.zbr:13
        const p = .{ @as(i64, 10), @as(i64, 20) };
// zbr:test/tuple_test.zbr:14
        std.debug.print("{}\n", .{p.@"0"});
// zbr:test/tuple_test.zbr:15
        std.debug.print("{}\n", .{p.@"1"});
// zbr:test/tuple_test.zbr:18
        const q: struct { i64, i64 } = .{ @as(i64, 3), @as(i64, 7) };
// zbr:test/tuple_test.zbr:19
        std.debug.print("{}\n", .{(q.@"0" + q.@"1")});
// zbr:test/tuple_test.zbr:22
        const _dt_54f0 = m.minMax(42, 17);
        const lo = _dt_54f0.@"0";
        const hi = _dt_54f0.@"1";
// zbr:test/tuple_test.zbr:23
        std.debug.print("{any}\n", .{lo});
// zbr:test/tuple_test.zbr:24
        std.debug.print("{any}\n", .{hi});
// zbr:test/tuple_test.zbr:27
        const t = .{ @as(i64, 1), @as(i64, 2), @as(i64, 3) };
// zbr:test/tuple_test.zbr:28
        std.debug.print("{}\n", .{t.@"2"});
    }

    pub fn init() Program {
        var self: Program = undefined;
        self._type_id = _tid_Program;
        return self;
    }
