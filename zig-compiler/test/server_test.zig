...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/server_test.zbr:4
        const handler = struct { fn call(req: HttpRequest) HttpResponse {
// zbr:test/server_test.zbr:5
            if (std.mem.eql(u8, req.path, "/")) {
// zbr:test/server_test.zbr:6
                return HttpResponse{ .status = 200, .text = "<h1>Hello from Zebra!</h1>" };
            }
// zbr:test/server_test.zbr:7
            if (std.mem.eql(u8, req.path, "/greet")) {
// zbr:test/server_test.zbr:8
                const msg: []const u8 = (std.fmt.allocPrint(_allocator, "Hello, {s} {s}", .{req.method, req.path}) catch @panic("OOM"));
// zbr:test/server_test.zbr:9
                return HttpResponse{ .status = 200, .text = msg };
            }
// zbr:test/server_test.zbr:10
            if (std.mem.eql(u8, req.path, "/echo")) {
// zbr:test/server_test.zbr:11
                return HttpResponse{ .status = 200, .text = req.content };
            }
// zbr:test/server_test.zbr:12
            return HttpResponse{ .status = 404, .text = "<h1>404 Not Found</h1>" };
         } }.call;
// zbr:test/server_test.zbr:13
        std.debug.print("{s}\n", .{"Listening on http://localhost:8080"});
// zbr:test/server_test.zbr:14
        _http_serve(8080, handler);
    }
