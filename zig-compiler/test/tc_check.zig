...
pub const TcStmtReturn = struct {
    value: *TcExpr = undefined,
    pub fn init(v: TcExpr) TcStmtReturn {
        var self: TcStmtReturn = undefined;
// zbr:test\tc_check.zbr:25
        { const _rp = _allocator.create(TcExpr) catch @panic("OOM"); _rp.* = v; self.value = _rp; }
        return self;
    }

};

pub const TcStmtVarDecl = struct {
    name: []const u8 = undefined,
    decl_type: TcType = undefined,
    init_expr: *TcExpr = undefined,
    pub fn init(n: []const u8, t: TcType, e: TcExpr) TcStmtVarDecl {
        var self: TcStmtVarDecl = undefined;
// zbr:test\tc_check.zbr:33
        self.name = n;
// zbr:test\tc_check.zbr:34
        self.decl_type = t;
// zbr:test\tc_check.zbr:35
        { const _rp = _allocator.create(TcExpr) catch @panic("OOM"); _rp.* = e; self.init_expr = _rp; }
        return self;
    }

};

pub const TcStmtAssign = struct {
    target_name: []const u8 = undefined,
    rhs_expr: *TcExpr = undefined,
    pub fn init(name: []const u8, rhs: TcExpr) TcStmtAssign {
        var self: TcStmtAssign = undefined;
// zbr:test\tc_check.zbr:42
        self.target_name = name;
// zbr:test\tc_check.zbr:43
        { const _rp = _allocator.create(TcExpr) catch @panic("OOM"); _rp.* = rhs; self.rhs_expr = _rp; }
        return self;
    }

};

pub const TcStmtIf = struct {
    cond: *TcExpr = undefined,
    pub fn init(c: TcExpr) TcStmtIf {
        var self: TcStmtIf = undefined;
// zbr:test\tc_check.zbr:49
        { const _rp = _allocator.create(TcExpr) catch @panic("OOM"); _rp.* = c; self.cond = _rp; }
        return self;
    }

};

pub const TcStmtAssert = struct {
    cond: *TcExpr = undefined,
    pub fn init(c: TcExpr) TcStmtAssert {
        var self: TcStmtAssert = undefined;
// zbr:test\tc_check.zbr:55
        { const _rp = _allocator.create(TcExpr) catch @panic("OOM"); _rp.* = c; self.cond = _rp; }
        return self;
    }
...
pub const TcCheck = struct {
    _type_tag: u64 = _ttag_TcCheck,
    errors: std.ArrayList([]const u8) = undefined,
    _infer: *TcInfer = undefined,
    _ret_type: TcType = undefined,
    pub fn init(infer: *TcInfer, ret_type: TcType) *TcCheck {
        const self = _allocator.create(TcCheck) catch @panic("OOM");
        self._type_tag = _ttag_TcCheck;
// zbr:test\tc_check.zbr:82
        self.errors = std.ArrayList([]const u8){};
// zbr:test\tc_check.zbr:83
        self._infer = infer;
// zbr:test\tc_check.zbr:84
        self._ret_type = ret_type;
        return self;
    }

    pub fn ok(self: *TcCheck) bool {
// zbr:test\tc_check.zbr:88
        return (@as(i64, @intCast(self.errors.items.len)) == 0);
    }

    pub fn report(self: *TcCheck, msg: []const u8) void {
// zbr:test\tc_check.zbr:92
        self.errors.append(_allocator, msg) catch unreachable;
    }

    pub fn checkStmt(self: *TcCheck, s: TcStmt) void {
// zbr:test\tc_check.zbr:96
        switch (s) {
            .return_ => |r| {
// zbr:test\tc_check.zbr:98
                const actual = self._infer.inferExpr(r.value.*);
// zbr:test\tc_check.zbr:99
                if ((!TcTypes.isAssignable(actual, self._ret_type))) {
// zbr:test\tc_check.zbr:100
                    self.report(_str_concat(_str_concat(_str_concat("return type mismatch: expected ", TcTypes.typeName(self._ret_type), _allocator), ", got ", _allocator), TcTypes.typeName(actual), _allocator));
                }
            },
            .return_void => {
// zbr:test\tc_check.zbr:103
                if ((!TcTypes.eql(self._ret_type, TcType{ .void_ = {} }))) {
// zbr:test\tc_check.zbr:104
                    if ((!TcTypes.eql(self._ret_type, TcType{ .unknown_ = {} }))) {
// zbr:test\tc_check.zbr:105
                        self.report(_str_concat(_str_concat("return without value in non-void method (expected ", TcTypes.typeName(self._ret_type), _allocator), ")", _allocator));
                    }
                }
            },
            .var_decl => |d| {
// zbr:test\tc_check.zbr:108
                const actual = self._infer.inferExpr(d.init_expr.*);
// zbr:test\tc_check.zbr:109
                if ((!TcTypes.isAssignable(actual, d.decl_type))) {
// zbr:test\tc_check.zbr:110
                    self.report(_str_concat(_str_concat(_str_concat(_str_concat(_str_concat("type mismatch in var '", d.name, _allocator), "': expected ", _allocator), TcTypes.typeName(d.decl_type), _allocator), ", got ", _allocator), TcTypes.typeName(actual), _allocator));
                }
            },
            .assign => |a| {
// zbr:test\tc_check.zbr:113
                const rhs = self._infer.inferExpr(a.rhs_expr.*);
// zbr:test\tc_check.zbr:114
                const lhs = self._infer.inferMember(TcType{ .unknown_ = {} }, a.target_name);
// zbr:test\tc_check.zbr:115
                if (TcTypes.eql(lhs, TcType{ .unknown_ = {} })) {
                    // pass
                } else {
// zbr:test\tc_check.zbr:118
                    if ((!TcTypes.isAssignable(rhs, lhs))) {
// zbr:test\tc_check.zbr:119
                        self.report(_str_concat(_str_concat(_str_concat(_str_concat(_str_concat("assign type mismatch for '", a.target_name, _allocator), "': expected ", _allocator), TcTypes.typeName(lhs), _allocator), ", got ", _allocator), TcTypes.typeName(rhs), _allocator));
                    }
                }
            },
            .if_ => |i| {
// zbr:test\tc_check.zbr:122
                const cond_t = self._infer.inferExpr(i.cond.*);
// zbr:test\tc_check.zbr:123
                if ((!TcTypes.eql(cond_t, TcType{ .bool_ = {} }))) {
// zbr:test\tc_check.zbr:124
                    if ((!TcTypes.eql(cond_t, TcType{ .unknown_ = {} }))) {
// zbr:test\tc_check.zbr:125
                        self.report(_str_concat("if condition must be bool, got ", TcTypes.typeName(cond_t), _allocator));
                    }
                }
            },
            .assert_ => |a| {
// zbr:test\tc_check.zbr:128
                const cond_t = self._infer.inferExpr(a.cond.*);
// zbr:test\tc_check.zbr:129
                if ((!TcTypes.eql(cond_t, TcType{ .bool_ = {} }))) {
// zbr:test\tc_check.zbr:130
                    if ((!TcTypes.eql(cond_t, TcType{ .unknown_ = {} }))) {
// zbr:test\tc_check.zbr:131
                        self.report(_str_concat("assert condition must be bool, got ", TcTypes.typeName(cond_t), _allocator));
                    }
                }
            },
            .pass_ => {
    ...
    pub fn checkStmts(self: *TcCheck, stmts: std.ArrayList(TcStmt)) void {
// zbr:test\tc_check.zbr:138
        var i: i64 = 0;
// zbr:test\tc_check.zbr:139
        while (_zebra_lt(i, @as(i64, @intCast(stmts.items.len)))) {
// zbr:test\tc_check.zbr:140
            self.checkStmt(stmts.items[@as(usize, @intCast(i))]);
// zbr:test\tc_check.zbr:141
            i = (i + 1);
        }
    }
...
const _ttag_TcCheck: u64 = 2192349176;
const _reflect_TcCheck_name: []const u8 = "TcCheck";
const _reflect_TcCheck_fields: []const []const u8 = &.{"errors", "_infer", "_ret_type"};
const _reflect_TcCheck_field_types: []const []const u8 = &.{"List(str)", "TcInfer", "TcType"};
