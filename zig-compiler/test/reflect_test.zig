...
pub const Product = struct {
    _type_tag: u64 = _ttag_Product,
    name: []const u8 = undefined,
    price: f64 = undefined,
    qty: i64 = undefined,
    pub fn init() Product {
        var self: Product = undefined;
        self._type_tag = _ttag_Product;
        return self;
    }

};

const _ttag_Product: u64 = 1385980108;
...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/reflect_test.zbr:9
        var p = Product.init();
// zbr:test/reflect_test.zbr:10
        p.name = "Widget";
// zbr:test/reflect_test.zbr:11
        p.price = 9.99;
// zbr:test/reflect_test.zbr:12
        p.qty = 42;
// zbr:test/reflect_test.zbr:15
        const cname = _reflect_Product_name;
// zbr:test/reflect_test.zbr:16
        if (!(std.mem.eql(u8, cname, "Product"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/reflect_test.zbr:19
        const fields = _reflect_Product_fields[0..];
// zbr:test/reflect_test.zbr:20
        if (!((@as(i64, @intCast(fields.len)) == 3))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/reflect_test.zbr:21
        if (!(std.mem.eql(u8, fields[@as(usize, @intCast(0))], "name"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/reflect_test.zbr:22
        if (!(std.mem.eql(u8, fields[@as(usize, @intCast(1))], "price"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/reflect_test.zbr:23
        if (!(std.mem.eql(u8, fields[@as(usize, @intCast(2))], "qty"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/reflect_test.zbr:26
        const types = _reflect_Product_field_types[0..];
// zbr:test/reflect_test.zbr:27
        if (!(std.mem.eql(u8, types[@as(usize, @intCast(0))], "str"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/reflect_test.zbr:28
        if (!(std.mem.eql(u8, types[@as(usize, @intCast(1))], "float"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/reflect_test.zbr:29
        if (!(std.mem.eql(u8, types[@as(usize, @intCast(2))], "int"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/reflect_test.zbr:32
        var joined: []const u8 = "";
// zbr:test/reflect_test.zbr:33
        for (_reflect_Product_fields[0..]) |f| {
// zbr:test/reflect_test.zbr:34
            joined = _str_concat(_str_concat(joined, f, _allocator), ";", _allocator);
        }
// zbr:test/reflect_test.zbr:35
        if (!(std.mem.eql(u8, joined, "name;price;qty;"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/reflect_test.zbr:37
        std.debug.print("{s}\n", .{"reflect_test: all assertions passed"});
    }
