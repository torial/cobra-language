...
pub const Product = struct {
    ...
    qty: i64,
    pub fn init() *Product {
        const self = _allocator.create(Product) catch @panic("OOM");
        self._type_tag = _ttag_Product;
        return self;
    }

};
const _ttag_Product: u64 = _zbr_hash("Product");
...
pub const Main = struct {
    ...
    pub fn main() void {
        ...
        p.price = 9.99;
        p.qty = 42;
        const cname = _reflect_lookup_name(p._type_tag);
        if (!(std.mem.eql(u8, cname, "Product"))) @panic("className");
        const fields = _reflect_lookup_fields(p._type_tag);
        if (!((@as(i64, @intCast(fields.items.len)) == 3))) @panic("field count");
        if (!(std.mem.eql(u8, fields.items[@intCast(0)], "name"))) @panic("field 0");
        if (!(std.mem.eql(u8, fields.items[@intCast(1)], "price"))) @panic("field 1");
        if (!(std.mem.eql(u8, fields.items[@intCast(2)], "qty"))) @panic("field 2");
        const types = _reflect_lookup_field_types(p._type_tag);
        if (!(std.mem.eql(u8, types.items[@intCast(0)], "str"))) @panic("type str");
        if (!(std.mem.eql(u8, types.items[@intCast(1)], "float"))) @panic("type float");
        if (!(std.mem.eql(u8, types.items[@intCast(2)], "int"))) @panic("type int");
        var joined: []const u8 = "";
        for (_reflect_lookup_fields(p._type_tag).items) |f| {
            joined = _str_concat(_str_concat(joined, f, _allocator), ";", _allocator);
        }
        if (!(std.mem.eql(u8, joined, "name;price;qty;"))) @panic("for-in fieldNames");
