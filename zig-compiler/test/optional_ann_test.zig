...
// Source: test/optional_ann_test.zbr
...
pub const Main = struct {
    pub fn findFirst(items: std.ArrayList([]const u8), prefix: []const u8) ?[]const u8 {
// zbr:test/optional_ann_test.zbr:8
        for (items.items) |item| {
// zbr:test/optional_ann_test.zbr:9
            if ((std.mem.indexOf(u8, item, prefix) != null)) {
// zbr:test/optional_ann_test.zbr:10
                return item;
            }
        }
// zbr:test/optional_ann_test.zbr:11
        return null;
    }

    pub fn main() void {
// zbr:test/optional_ann_test.zbr:14
        var items = std.ArrayList([]const u8){};
        defer items.deinit(_allocator);
// zbr:test/optional_ann_test.zbr:15
        items.append(_allocator, "apple") catch unreachable;
// zbr:test/optional_ann_test.zbr:16
        items.append(_allocator, "banana") catch unreachable;
// zbr:test/optional_ann_test.zbr:17
        items.append(_allocator, "cherry") catch unreachable;
// zbr:test/optional_ann_test.zbr:20
        const found = Main.findFirst(items, "ban");
// zbr:test/optional_ann_test.zbr:21
        if ((found != null)) {
// zbr:test/optional_ann_test.zbr:22
            std.debug.print("{s}\n", .{"found"});
        } else {
// zbr:test/optional_ann_test.zbr:24
            std.debug.print("{s}\n", .{"not found"});
        }
// zbr:test/optional_ann_test.zbr:26
        const missing = Main.findFirst(items, "xyz");
// zbr:test/optional_ann_test.zbr:27
        if ((missing == null)) {
// zbr:test/optional_ann_test.zbr:28
            std.debug.print("{s}\n", .{"correctly nil"});
        } else {
// zbr:test/optional_ann_test.zbr:30
            std.debug.print("{s}\n", .{"unexpected"});
        }
// zbr:test/optional_ann_test.zbr:32
        std.debug.print("{s}\n", .{"optional_ann_test: done"});
    }
