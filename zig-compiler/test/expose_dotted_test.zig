...
// Source: test/expose_dotted_test.zbr
...
pub fn _initAllocator(a: std.mem.Allocator) void {
    _allocator = a;
    @import("expose_dotted/lib.zig")._initAllocator(a);
}
...
const lib = @import("expose_dotted/lib.zig");
const DottedLib = lib.DottedLib;
const Point = lib.Point;
pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
// zbr:test/expose_dotted_test.zbr:23
        const g = DottedLib.greet();
// zbr:test/expose_dotted_test.zbr:24
        if (!(std.mem.eql(u8, g, "hello from dotted lib"))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/expose_dotted_test.zbr:25
        std.debug.print("{s}\n", .{"1. dotted.lib static str call: OK"});
// zbr:test/expose_dotted_test.zbr:28
        const sum = DottedLib.add(7, 3);
// zbr:test/expose_dotted_test.zbr:29
        if (!((sum == 10))) {
            std.debug.print("assertion failed\n", .{});
            unreachable;
        }
// zbr:test/expose_dotted_test.zbr:30
        std.debug.print("{s}\n", .{"2. dotted.lib static int call: OK"});
// zbr:test/expose_dotted_test.zbr:32
        std.debug.print("{s}\n", .{"expose_dotted_test: all OK"});
    }
...
pub fn main() void {
    ...
    defer _arena.deinit();
    @import("expose_dotted/lib.zig")._initAllocator(_allocator);
    Main.main();
}
