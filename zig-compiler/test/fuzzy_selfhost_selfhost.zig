...
// Source: test/fuzzy_selfhost.zbr
...
pub const Program = struct {
    ...
    pub fn buildNgrams(text: []const u8) std.StringHashMap(i64) {
        ...
        var clist = std.ArrayList([]const u8){};
        {
            ...
            while (_cp_it_c.nextCodepoint()) |c| {
                clist.append(_allocator, (blk: { var _cpbuf: [4]u8 = undefined; const _cplen = std.unicode.utf8Encode(@intCast(c), &_cpbuf) catch 1; break :blk _allocator.dupe(u8, _cpbuf[0.._cplen]) catch @panic("OOM"); })) catch @panic("OOM");
            }
        }
        var i: i64 = 0;
        while ((i < (@as(i64, @intCast(clist.items.len)) - 2))) {
            const tri = (std.mem.concat(_allocator, u8, &.{ (std.mem.concat(_allocator, u8, &.{ clist.items[@intCast(i)], clist.items[@intCast((i + 1))] }) catch unreachable), clist.items[@intCast((i + 2))] }) catch unreachable);
            if (ngrams.contains(tri)) {
                ngrams.put(tri, ((ngrams.get(tri) orelse undefined) + 1)) catch @panic("OOM");
            } else {
                ngrams.put(tri, 1) catch @panic("OOM");
            }
            i = (i + 1);
        }
        return ngrams;
    ...
    pub fn intersection(a: std.StringHashMap(i64), b: std.StringHashMap(i64)) i64 {
        var total: i64 = 0;
        {
            ...
            while (_it_tri.next()) |_ent_tri| {
                const tri = _ent_tri.key_ptr.*;
                const ca = _ent_tri.value_ptr.*;
                if (b.contains(tri)) {
    ...
    pub fn loadVerses(path: []const u8) std.ArrayList([]const u8) {
        ...
        const lines: std.ArrayList([]const u8) = (blk_frl: {
        ...
        for (lines.items) |line| {
            if (first) {
                ...
                if ((@as(i64, @intCast(parts.items.len)) >= 3)) {
                    const text: []const u8 = parts.items[@intCast(2)];
                    if ((@as(i64, @intCast(std.unicode.utf8CountCodepoints(text) catch 0)) > 4)) {
                        verses.append(_allocator, text) catch @panic("OOM");
                    }
                }
            }
        }
    ...
    pub fn main() void {
        ...
        std.debug.print("{s}\n", .{"Mark verses: "});
        std.debug.print("{}\n", .{@as(i64, @intCast(mark_verses.items.len))});
        std.debug.print("{s}\n", .{"Mat verses: "});
        std.debug.print("{}\n", .{@as(i64, @intCast(mat_verses.items.len))});
        var mat_ngrams = std.ArrayList(std.StringHashMap(i64)){};
        var mat_totals = std.ArrayList(i64){};
        var mat_idx = std.ArrayList(i64){};
        var pi: i64 = 0;
        while ((pi < @as(i64, @intCast(mat_verses.items.len)))) {
            const ptext: []const u8 = mat_verses.items[@intCast(pi)];
            const ptotal = (@as(i64, @intCast(std.unicode.utf8CountCodepoints(ptext) catch 0)) - 2);
            if ((ptotal > 3)) {
                mat_ngrams.append(_allocator, Program.buildNgrams(ptext)) catch @panic("OOM");
                mat_totals.append(_allocator, ptotal) catch @panic("OOM");
                mat_idx.append(_allocator, pi) catch @panic("OOM");
            }
            pi = (pi + 1);
        }
        std.debug.print("{s}\n", .{"Mat ngrams cached: "});
        std.debug.print("{}\n", .{@as(i64, @intCast(mat_ngrams.items.len))});
        var mi: i64 = 0;
        while ((mi < @as(i64, @intCast(mark_verses.items.len)))) {
            ...
            if ((total_m > 3)) {
                ...
                var ti: i64 = 0;
                while ((ti < @as(i64, @intCast(mat_ngrams.items.len)))) {
                    const total_t: i64 = mat_totals.items[@intCast(ti)];
                    const ngrams_t: std.StringHashMap(i64) = mat_ngrams.items[@intCast(ti)];
...
const _ttag_Program: u64 = _zbr_hash("Program");
