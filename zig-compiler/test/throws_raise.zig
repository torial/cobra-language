...
// zbr:test/throws_raise.zbr:5
            { const _rdet_2088: []const u8 = std.fmt.allocPrint(_allocator, "{}", .{x}) catch @panic("OOM");
              ...
              const _rshim_2088 = struct { fn call(p: *anyopaque) []const u8 {
                  return @as(*[]const u8, @alignCast(@ptrCast(p))).*;
              } };
              _error_ctx = .{ .message = "negative input", .details = .{ .ptr = @ptrCast(_rdet_ptr_2088), .toString_fn = _rshim_2088.call } };
            }
