...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/interp_test.zbr:4
        const name = "world";
// zbr:test/interp_test.zbr:5
        const n = 42;
// zbr:test/interp_test.zbr:6
        {
            const _pt0 = (std.fmt.allocPrint(_allocator, "hello {s}!", .{name}) catch @panic("OOM"));
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/interp_test.zbr:7
        {
            const _pt0 = (std.fmt.allocPrint(_allocator, "n = {}", .{n}) catch @panic("OOM"));
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/interp_test.zbr:8
        const msg: []const u8 = (std.fmt.allocPrint(_allocator, "result: {} and {s}", .{n, name}) catch @panic("OOM"));
        defer _allocator.free(msg);
// zbr:test/interp_test.zbr:9
        std.debug.print("{s}\n", .{msg});
    }
