...
// zbr:test/raise_auto.zbr:4
        if (_zebra_lt(x, 0)) {
// zbr:test/raise_auto.zbr:5
            { const _rdet_2e88: []const u8 = std.fmt.allocPrint(_allocator, "{}", .{x}) catch @panic("OOM");
              const _rdet_ptr_2e88 = _allocator.create([]const u8) catch @panic("OOM");
              _rdet_ptr_2e88.* = _rdet_2e88;
              const _rshim_2e88 = struct { fn call(p: *anyopaque) []const u8 {
                  return @as(*[]const u8, @alignCast(@ptrCast(p))).*;
              } };
              _error_ctx = .{ .message = "negative value", .details = .{ .ptr = @ptrCast(_rdet_ptr_2e88), .toString_fn = _rshim_2e88.call } };
            }
            return error.ZebraError;
        }
// zbr:test/raise_auto.zbr:6
        return (x * 2);
    }

    pub fn main() void {
// zbr:test/raise_auto.zbr:9
        var result: i64 = 0;
// zbr:test/raise_auto.zbr:10
        var _try_err_29a8: ?anyerror = null;
        _try_blk_29a8: {
// zbr:test/raise_auto.zbr:11
            result = App.risky(5) catch |_tc_24d8ce12938| { _try_err_29a8 = _tc_24d8ce12938; break :_try_blk_29a8; };
            break :_try_blk_29a8;
        }
        if (_try_err_29a8 != null) {
// zbr:test/raise_auto.zbr:13
            result = (-1);
        }
// zbr:test/raise_auto.zbr:14
        std.debug.print("{}\n", .{result});
    }
