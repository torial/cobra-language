...
// zbr:test/fn_ref_test.zbr:2
    return ((_zebra_ge(c, 'a') and _zebra_le(c, 'z')) or (_zebra_ge(c, 'A') and _zebra_le(c, 'Z')));
}

pub fn isDigit(c: u21) bool {
// zbr:test/fn_ref_test.zbr:5
    return (_zebra_ge(c, '0') and _zebra_le(c, '9'));
}

pub const FnRefTest = struct {
    _type_tag: u64 = _ttag_FnRefTest,
    pub fn run(self: *FnRefTest) void {
        _ = self;
// zbr:test/fn_ref_test.zbr:10
        const f = isAlpha;
// zbr:test/fn_ref_test.zbr:11
        std.debug.assert(f('x'));
// zbr:test/fn_ref_test.zbr:12
        std.debug.assert((!f('3')));
// zbr:test/fn_ref_test.zbr:14
        const g = isDigit;
// zbr:test/fn_ref_test.zbr:15
        std.debug.assert(g('7'));
// zbr:test/fn_ref_test.zbr:16
        std.debug.assert((!g('z')));
// zbr:test/fn_ref_test.zbr:19
        var pred: @TypeOf(&isAlpha) = &isAlpha;
// zbr:test/fn_ref_test.zbr:20
        std.debug.assert(pred('A'));
// zbr:test/fn_ref_test.zbr:21
        pred = &isDigit;
// zbr:test/fn_ref_test.zbr:22
        std.debug.assert(pred('5'));
// zbr:test/fn_ref_test.zbr:23
        std.debug.assert((!pred('A')));
    }

    pub fn init() FnRefTest {
        var self: FnRefTest = undefined;
        self._type_tag = _ttag_FnRefTest;
        return self;
    }

};

const _ttag_FnRefTest: u64 = 1166496498;
const _reflect_FnRefTest_name: []const u8 = "FnRefTest";
const _reflect_FnRefTest_fields: []const []const u8 = &.{};
const _reflect_FnRefTest_field_types: []const []const u8 = &.{};

pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
// zbr:test/fn_ref_test.zbr:27
        var t = FnRefTest.init();
// zbr:test/fn_ref_test.zbr:28
        t.run();
// zbr:test/fn_ref_test.zbr:29
        std.debug.print("{s}\n", .{"fn_ref_test: all OK"});
    }
