...
// Source: test/generic_constrained_test.zbr
...
pub fn Comparable(comptime T: type) void {
    comptime {
        if (!@hasDecl(T, "compareTo")) @compileError("type " ++ @typeName(T) ++ " does not implement Comparable.compareTo");
    }
}

pub const Score = struct {
    _type_tag: u64 = _ttag_Score,
    value: i64 = undefined,
    pub fn init(v: i64) Score {
        var self: Score = undefined;
        self._type_tag = _ttag_Score;
// zbr:test/generic_constrained_test.zbr:8
        self.value = v;
        return self;
    }

    pub fn compareTo(self: *Score, other: Score) i64 {
// zbr:test/generic_constrained_test.zbr:11
...
const _ttag_Score: u64 = 2011890229;
const _reflect_Score_name: []const u8 = "Score";
const _reflect_Score_fields: []const []const u8 = &.{"value"};
const _reflect_Score_field_types: []const []const u8 = &.{"int"};

pub fn MinHeap(comptime T: type) type {
    return struct {
        _type_tag: u64 = _ttag_MinHeap,
        best: T = undefined,
        pub fn init(seed: T) @This() {
            var self: @This() = undefined;
            self._type_tag = _ttag_MinHeap;
// zbr:test/generic_constrained_test.zbr:19
            self.best = seed;
            return self;
        }

        pub fn top(self: *@This()) T {
// zbr:test/generic_constrained_test.zbr:22
            return self.best;
        }
...
const _ttag_MinHeap: u64 = 1507774993;

pub const Main = struct {
    _type_tag: u64 = _ttag_Main,
    pub fn main() void {
// zbr:test/generic_constrained_test.zbr:28
        var h = MinHeap(Score).init(Score.init(42));
// zbr:test/generic_constrained_test.zbr:29
        std.debug.print("{}\n", .{h.top().value});
// zbr:test/generic_constrained_test.zbr:30
        std.debug.print("{s}\n", .{"constrained generics OK"});
    }
