...
// Source: test/capture_boundary.zbr
...
pub const Program = struct {
    ...
    pub fn main() void {
        const double = struct { fn call(x: i64) @TypeOf((x * 2)) { return (x * 2); } }.call;
        const offset: i64 = 10;
        const adder = (struct {
            offset: i64,
...
fn _reflect_lookup_name(tag: u64) []const u8 {
    if (tag == _ttag_Program) return _reflect_Program_name;
    return "unknown";
}
fn _reflect_lookup_fields(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Program) return .{ .items = _reflect_Program_fields };
    return .{ .items = &.{} };
}
fn _reflect_lookup_field_types(tag: u64) _ReflectStrSlice {
    if (tag == _ttag_Program) return .{ .items = _reflect_Program_field_types };
    return .{ .items = &.{} };
}
