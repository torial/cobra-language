...
// extend String
fn _ext_String_shout(self: []const u8) []const u8 {
// zbr:test/extend_test.zbr:3
    {
        const _ret_recv = (std.ascii.allocUpperString(_allocator, self) catch unreachable);
        defer _allocator.free(_ret_recv);
        return (std.mem.concat(_allocator, u8, &.{ _ret_recv, "!" }) catch unreachable);
    }
}

fn _ext_String_isLong(self: []const u8) bool {
// zbr:test/extend_test.zbr:6
    return _zebra_gt(self.len, 5);
}

fn _ext_String_greet(self: []const u8) []const u8 {
// zbr:test/extend_test.zbr:9
    {
        const _ret_recv = (std.mem.concat(_allocator, u8, &.{ "Hello, ", self }) catch unreachable);
        defer _allocator.free(_ret_recv);
        return (std.mem.concat(_allocator, u8, &.{ _ret_recv, "!" }) catch unreachable);
    }
}
...
pub const Main = struct {
    ...
    pub fn main() void {
// zbr:test/extend_test.zbr:14
        const s: []const u8 = "hello";
// zbr:test/extend_test.zbr:15
        const t: []const u8 = "world";
// zbr:test/extend_test.zbr:16
        {
            const _pt0 = _ext_String_shout(s);
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
// zbr:test/extend_test.zbr:17
        std.debug.print("{}\n", .{_ext_String_isLong(s)});
// zbr:test/extend_test.zbr:18
        {
            const _pt0 = _ext_String_greet(t);
            defer _allocator.free(_pt0);
            std.debug.print("{s}\n", .{_pt0});
        }
    }
